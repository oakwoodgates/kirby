        ...
        for row in result:
            snapshot = {
                ...
                "candle": {
                    ...
                    "open": float(row[8]) if row[8] else None,
                    "high": float(row[9]) if row[9] else None,
                    "low": float(row[10]) if row[10] else None,
                    "close": float(row[11]) if row[11] else None,
                    "volume": float(row[12]) if row[12] else None,
                } if row[7] else None,
                "funding": {
                    "timestamp": row[13].isoformat() if row[13] else None,
                    "rate": float(row[14]) if row[14] else None,
                    "mark_price": float(row[15]) if row[15] else None,
                    "next_funding_time": row[16].isoformat() if row[16] else None,
                } if row[13] else None,
                "open_interest": {
                    "timestamp": row[17].isoformat() if row[17] else None,
                    "value": float(row[18]) if row[18] else None,
                } if row[17] else None,
